...
for j in range(1,9):
    env = Environment(experiment_name=experiment_name,
				  ...
				  level=2,
				  visuals=False)
    best, mean_stat, peak_stat = test_mutation(0.2, 1.0)
    bests.append(best)
    means.append(mean_stat)
    peaks.append(peak_stat)

np.savetxt("All_8_Params.csv", bests, delimiter=",")
np.savetxt("All_8_Means.csv", means, delimiter=",")
np.savetxt("All_8_Peaks.csv", peaks, delimiter=",")
...
bests2 = pd.read_csv("All_8_Params.csv", delimiter=",", header=None)
performance = []
print(bests2)
for j in range(1,9):
